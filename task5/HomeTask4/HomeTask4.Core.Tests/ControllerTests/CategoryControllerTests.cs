...
namespace HomeTask4.Core.Tests.ControllerTests
{
    public class CategoryControllerTests
    {
        ...
        public async Task TryCreateCategoryAsyncTest_Throws_Exception_On_Empty_Name()
        {
            ...
            repos.Setup(r => r.SingleOrDefaultAsync<Category>(It.IsAny<Expression<Func<Category, bool>>>()));
            ...
            repos.Verify(r => r.SingleOrDefaultAsync<Category>(It.IsAny<Expression<Func<Category, bool>>>()), Times.Never);
        ...
        public async Task TryCreateCategoryAsyncTest_Returns_Bool()
        {
            ...
            repos.Setup(r => r.SingleOrDefaultAsync<Category>(x => string.Equals(x.Name, cat.Name, StringComparison.OrdinalIgnoreCase) && x.ParentId == cat.ParentId))
            ...
            repos.Setup(r => r.SingleOrDefaultAsync<Category>(x => string.Equals(x.Name, cat2.Name, StringComparison.OrdinalIgnoreCase) && x.ParentId == cat2.ParentId))
            ...
            repos.Verify(repos => repos.SingleOrDefaultAsync<Category>(It.IsAny<Expression<Func<Category, bool>>>()), Times.Exactly(2));
            Assert.True(result);
        ...
        public async Task CreateCategoryAsyncTest_Throws_ArgumentException()
        {
            ...
            repos.Setup(r => r.AddAsync<Category>(It.IsAny<Category>())).Callback((Category some) => mockDB.Add(some));
            repos.Setup(r => r.GetByIdAsync<Category>(It.IsAny<int>())).ReturnsAsync((int a) => mockDB.FirstOrDefault(x => x.Id == a));
            repos.Setup(r => r.SingleOrDefaultAsync<Category>(x => string.Equals(x.Name, cat1.Name, StringComparison.OrdinalIgnoreCase) && x.ParentId == cat1.ParentId))
            ...
            repos.Verify(repos => repos.GetByIdAsync<Category>(It.IsAny<int>()), Times.Never);
            repos.Verify(r => r.SingleOrDefaultAsync<Category>(It.IsAny<Expression<Func<Category, bool>>>()), Times.Exactly(1));
